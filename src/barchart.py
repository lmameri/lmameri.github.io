...
COLORS = ['rgb(243, 231, 155)', 'rgb(248, 160, 126)',
          'rgb(206, 102, 147)', 'rgb(92, 83, 165)']
WIDTH = 1000
HEIGHT = 700
...
def get_barchart():
    ...
    fig.add_traces([
        go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=True,
               name='Photo', x=df_photo['date'], y=df_photo['count'], marker={'color': COLORS[0]}),
        go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=True,
               name='Video', x=df_video['date'], y=df_video['count'], marker={'color': COLORS[1]}),
        go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=True,
               name='Album', x=df_album['date'], y=df_album['count'], marker={'color': COLORS[2]}),
        go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=True,
               name='IGTV', x=df_igtv['date'], y=df_igtv['count'], marker={'color': COLORS[3]})
    ])
    ...
    for media in media_list:
        df_photo, df_video, df_album, df_igtv = preprocess.preprocess_barchart_account(
            media)
        fig.add_traces([
            go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=False,
                   name='Photo', x=df_photo['date'], y=df_photo['count'], marker={'color': COLORS[0]}),
            go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=False,
                   name='Video', x=df_video['date'], y=df_video['count'], marker={'color': COLORS[1]}),
            go.Bar(hovertemplate=hovertemplate.get_hovertemplate_barchart(), visible=False,
